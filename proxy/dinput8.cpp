...
typedef HRESULT(WINAPI* CreateInputDevice_t)(IDirectInput8A*,
                                             REFGUID,
...
inline const GUID MyGUID_SysMouse             = {
    0x6F1D2B60, 0xD5A0, 0x11CF, {0xBF, 0xC7, 0x44, 0x45, 0x53, 0x54, 0x00, 0x00}};
inline const GUID MyGUID_SysKeyboard = {
...
FARPROC
GetOriginalFunction(const char* funcName)
{
    LoadOriginalDInput8();
    ...
    {
        MessageBox(NULL,
                   ConvertToWide("Failed to resolve function: " + std::string(funcName)).c_str(),
                   L"Error",
                   MB_OK);
    }
...
HRESULT WINAPI
HookedCreateInputDevice(IDirectInput8A* pDirectInput,
                        REFGUID rguid,
...
{
    auto DirectInput8CreateFunc =
        reinterpret_cast<HRESULT(WINAPI*)(HINSTANCE, DWORD, REFIID, LPVOID*, LPUNKNOWN)>(
    ...
    if (MH_CreateHook(vtable[3],
                      HookedCreateInputDevice,
                      reinterpret_cast<void**>(&originalCreateInputDevice)) == MH_OK)
    {
...
extern "C" __declspec(dllexport) HRESULT WINAPI
Proxy_DllGetClassObject(REFCLSID rclsid, REFIID riid, LPVOID* ppv)
{
    return reinterpret_cast<HRESULT(WINAPI*)(REFCLSID, REFIID, LPVOID*)>(
        GetOriginalFunction("DllGetClassObject"))(rclsid, riid, ppv);
}
...
extern "C" __declspec(dllexport) LPCDIDATAFORMAT WINAPI
