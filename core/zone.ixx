...
void AssignIfPresent(const rapidjson::Value::ConstObject& obj, const char* fieldName, int& target) {
    if (obj.HasMember(fieldName) && obj[fieldName].IsInt()) {
        target = obj[fieldName].GetInt();
    }
}

void AssignIfPresent(const rapidjson::Value::ConstObject& obj, const char* fieldName, std::string& target) {
...
ZoneConfigEntry CreateZoneConfig(const rapidjson::Value::ConstObject& value, const ZoneConfigEntry& defaults) {
...
using zone_entry_add_t = uintptr_t(__fastcall*)(uintptr_t This, uintptr_t Reg, int zone_type, int zone_id, const char* short_name, const char* long_name, int str_id, int zone_flags, int x, int y, int z);
...
private:
    ...
    static void SetupHooks() {
        ...
        Hooks = {
          {reinterpret_cast<LPVOID>(eqlib::EQGameBaseAddress + 0x3DCA10), &ZoneConfig::ZoneCtor, reinterpret_cast<LPVOID*>(&Original_ZoneCtor)},
          {reinterpret_cast<LPVOID>(eqlib::EQGameBaseAddress + 0x3DC290), &ZoneConfig::ZoneEntryCtor, reinterpret_cast<LPVOID*>(&Original_ZoneEntryCtor)},
          {reinterpret_cast<LPVOID>(eqlib::EQGameBaseAddress + 0x3D0660), &ZoneConfig::GetTableString, reinterpret_cast<LPVOID*>(&Original_GetTableString)},
        };
    ...
    static ZoneManager* __fastcall ZoneCtor(ZoneManager* This, uintptr_t Reg) {
        ...
        // We've run through the constructor for ZoneManager which means we've added all the zones internal to eqgame.exe
        // and updated any along the way with our updated list. Now we can add our own.
        for (const auto& entry : AddZones) {
            if (entry.zoneId == -1) {
                continue;
            }
            Original_AddZoneEntry((uintptr_t)ZoneManagerInstance, 0,
                entry.zoneType,
                entry.zoneId,
    ...
    static uintptr_t __fastcall ZoneEntryCtor(uintptr_t This, uintptr_t Reg, int zone_type, int zone_id, const char* short_name, const char* long_name, int str_id, int zone_flags, int x, int y, int z) {
        for (const auto& entry : UpdateZones) {
            if (short_name == entry.shortName) {
                // -1 is sentinel value for default values, i.e. take what we already have provided, otherwise take the provided value
                return Original_ZoneEntryCtor(This, Reg, 
