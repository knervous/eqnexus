...
export class NpcConfig
{
   public:
    static void Init(const rapidjson::Value::Array& npcs)
    {
        for (const auto& value : npcs)
        {
            ...
            AddNpcs.push_back({value["raceId"].GetInt(),
   ...
   private:
    ...
    static void SetupHooks()
    {
        ...
        Hooks           = {
            {reinterpret_cast<LPVOID>(eqlib::EQGameBaseAddress + 0x10A5F0),
                       &NpcConfig::AddNpcCtor,
                       reinterpret_cast<LPVOID*>(&Original_NpcManagerCtor)},
        };
