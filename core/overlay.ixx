   ...
   private:
            ...
            if (!document.HasParseError())
            {
                ...
                if (core_version != manifest_version && !version_warning_shown)
                {
                    ...
                    std::thread t([this, manifest_version, release_url]() {
                        ...
                        ShowPopup(
                            "EQ Nexus Core version out of date. Latest version is: " +
                            manifest_version +
                            "<br></br><br></br>To download the latest core version, please visit " +
                            "<a href=\"" + release_url + "\">" + release_url + "</a>");
                    });
                    t.detach();
                }
                ...
                for (const auto& server : document["servers"].GetArray())
                {
                    ...
                    {
                        if (auto serverResponse =
                                http::DownloadJson(server["manifest"].GetString());
                            !serverResponse.empty())
                        {
                            ...
                            if (serverDocument.HasMember("files") &&
                                serverDocument["files"].IsObject())
                            {
                            ...
                            auto getStringOrDefault =
                                [&](const char* key,
                                    const std::string& defaultValue) -> std::string {
                                return serverDocument.HasMember(key) &&
                                               serverDocument[key].IsString()
                                           ? serverDocument[key].GetString()
                                           : defaultValue;
                            };

                            servers.emplace_back(std::make_unique<ServerInfo>(
                                getStringOrDefault("shortName", ""),
    ...
    void OnRender(IDirect3DDevice9* device)
    {
        ...
        nk_style_push_style_item(
            ctx, &s->window.fixed_background, nk_style_item_color(nk_rgb(bg.r, bg.g, bg.b)));

        struct nk_rect window_rect = nk_rect(window_x, window_y, window_width, window_height);
        struct nk_rect window_rect_popup =
            nk_rect(window_x - window_width - 20, window_y, window_width, 150);
                         ...
                         NK_WINDOW_MINIMIZABLE | NK_WINDOW_TITLE))
        {
            ...
            if (server_op)
            {
                ...
                {
                    ServerInfo info;
                    info.CreateManifest([this](const std::string& name) {
                        file_processed = name.empty() ? "" : "Hashing: " + name;
                    });
                }
    ...
    void InitNuklearCtx(IDirect3DDevice9* device)
    {
        ...
        if (SUCCEEDED(device->GetViewport(&viewport)))
        {
            ...
            s->window.header.label_normal     = s->window.header.label_hover =
                s->window.header.label_active = nk_rgb(255, 218, 96);
            s->window.header.align            = NK_HEADER_RIGHT;
            s->window.header.normal           = s->window.header.active =
                nk_style_item_color(nk_rgb(41, 56, 74));
    ...
    void PollDirectInputForNuklear()
    {
        if (mouse && SUCCEEDED(mouse->GetDeviceState(sizeof(DIMOUSESTATE), (LPVOID) &mouse_state)))
        {
            ...
            nk_input_button(
                ctx, NK_BUTTON_LEFT, mouse_x, mouse_y, (mouse_state.rgbButtons[0] & 0x80) != 0);
            nk_input_button(
                ctx, NK_BUTTON_RIGHT, mouse_x, mouse_y, (mouse_state.rgbButtons[1] & 0x80) != 0);
        }
    }
