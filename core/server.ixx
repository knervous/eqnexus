...
public:
	...
	static void RemoveCallback(const std::string& remove_id) {
		ContextCallbacks.erase(
			std::remove_if(ContextCallbacks.begin(), ContextCallbacks.end(),
	...
	static void Teardown() {
		// Fire one last set of callbacks - other hooks depend on change listeners for their own init/teardown
		SetContext("");
		ContextCallbacks.clear();
	}
	static const std::string& GetContext() { return Context; }
