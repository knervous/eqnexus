...
inline uintptr_t
FixEQGameOffset(T nOffset)
{
    return static_cast<uintptr_t>(nOffset) - static_cast<uintptr_t>(EQGamePreferredAddress) +
           EQGameBaseAddress;
}

template <typename T, typename = std::enable_if_t<std::is_integral_v<T>, void>>
inline uintptr_t
FixEQMainOffset(T nOffset)
{
    return static_cast<uintptr_t>(nOffset) - static_cast<uintptr_t>(EQLibraryPreferredAddress) +
           EQMainBaseAddress;
}
...
InitializeEQMainOffsets()
{
    ...
    EQMain__CEQSuiteTextureLoader__GetTexture =
        FixEQMainOffset(EQMain__CEQSuiteTextureLoader__GetTexture_x);
    ...
    EQMain__pinstLoginClient =
        FixEQMainOffset(EQMain__pinstCLoginViewManager_x) - sizeof(uintptr_t);

    if (EQMain__LoginController__GiveTime)
    {
#if defined(_M_AMD64)
        EQMain__LoginController__ProcessKeyboardEvents =
            GetFunctionAddressAt(EQMain__LoginController__GiveTime + 9, 1, 4);
        EQMain__LoginController__ProcessMouseEvents =
            GetFunctionAddressAt(EQMain__LoginController__GiveTime + 22, 1, 4);
#else
        // EQMain__LoginController__ProcessKeyboardEvents =
        // GetFunctionAddressAt(EQMain__LoginController__GiveTime + 3, 1, 4);
        // EQMain__LoginController__ProcessMouseEvents =
        // GetFunctionAddressAt(EQMain__LoginController__GiveTime + 11, 1, 4);
#endif
    }
